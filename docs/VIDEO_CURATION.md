...
- `approved`：已通過**策展審閱**——範圍框限、來源與講者資格、文獻對應與課程編排均經策展人確認。
  **不代表策展人對第三方影片的臨床內容做醫療背書**，臨床責任屬原發布單位與講者。
  技術連結驗證本身不會自動提升到此狀態
...
### ⚠️ 待策展人裁決：`R1kG9Mu1at4` 的 on-track／off-track 反向敘述

**片**：MRA 盂唇判讀與正常變異（ch7-u3）
**位置**：21:58–22:43

講者說明 Hill-Sachs 病灶的 on-track／off-track 時，**連續三次一致地**表示
「on-track 較容易再咬合（reengagement）、比 off-track 更需緊急手術介入」。

這與現行 glenoid track 概念相反——**off-track** 才是病灶越出軌道、會咬合、
再脫位風險高而需手術處置（Remplissage／Latarjet）的一類。

**判定為影片內容問題，不是字幕訛誤**：該段字幕清晰可辨，且全片只在此處講此概念，
三次敘述彼此一致，排除單次口誤。

**已做的處置**：逐段筆記不採用該段的方向與風險敘述，只保留骨缺損量測方法
（最佳擬合圓、橫截面比例、梯形化與骨膜剝離徵象），並在該片 `note` 明確記載。

**待裁決（三選一）**：
1. 保留影片，在 ch7-u3 的 `scope_note` 加一句提醒學員該段敘述與現行定義相反
2. 縮 `diagnostic_segment_range` 跳過 21:53–25:35，逐段筆記一併移除該段
3. 移除該片，另尋盂唇 MRA 片源

第 1 案成本最低且保留其餘教學價值；第 2 案會連帶失去同段的骨缺損量測內容。

**方法留存**：這類錯誤**不會被任何自動測試擋下**，只有主編回原逐字稿抽驗才抓得到。
新站的逐段筆記階段，撰稿 agent 標為「敘述前後有疑義」的段落**一律要人工回讀原文**——
本例中 agent 的標註是對的，但它把原因歸給「字幕不清」，實際是影片講錯。
